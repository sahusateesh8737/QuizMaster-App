...
from .models import Category, Question, QuestionOption, Quiz, QuizAttempt
...
class TestQuizFlow:
    ...
    def question(self, quiz):
        question = Question.objects.create(quiz=quiz, text="What is 2+2?", type="mcq", order=1)
        QuestionOption.objects.create(question=question, text="4", is_correct=True)
        QuestionOption.objects.create(question=question, text="5", is_correct=False)
        return question
    ...
    def test_submit_answer(self, client, user, quiz, question):
        ...
        data = {"question_id": question.id, "selected_option_id": correct_option.id}
