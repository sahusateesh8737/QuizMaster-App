...
class LiveQuizSessionAdmin(admin.ModelAdmin):
    list_display = ['join_code', 'quiz', 'host', 'status', 'total_participants', 'created_at']
...
class LiveQuizAnswerAdmin(admin.ModelAdmin):
    list_display = ['participant', 'question', 'is_correct', 'points_awarded', 'time_taken', 'answered_at']
...
class LiveQuizQuestionResultAdmin(admin.ModelAdmin):
    list_display = ['session', 'question', 'total_answers', 'correct_count', 'wrong_count', 'average_time']
