...
class LiveQuizSessionViewSet(viewsets.ModelViewSet):
    ...
    def next_question(self, request, pk=None):
        ...
        if current_question:
            result, created = LiveQuizQuestionResult.objects.get_or_create(session=session, question=current_question)
    ...
    def join(self, request):
        ...
        if user:
            participant, created = LiveQuizParticipant.objects.get_or_create(
                session=session, user=user, defaults={"status": "active"}
            )
        else:
            if not nickname:
                return Response(
            ...
            participant = LiveQuizParticipant.objects.create(session=session, nickname=nickname, status="active")
            created = True
...
class LiveQuizParticipantViewSet(viewsets.ModelViewSet):
    ...
    def submit_answer(self, request, pk=None):
        ...
        try:
            ...
            return Response({"detail": "Invalid question."}, status=status.HTTP_404_NOT_FOUND)
